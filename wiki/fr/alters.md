...
Le système d'`alters` du modèle [`Documents`](models.md) permet d'appliquer des **transformations post-fetch** aux données ramenées par PDO, avant que le modèle ne les retourne à l'appelant. C'est l'outil qui sert à normaliser les types (`'1'` → `1`), à hydrater des références (`country_code = 'FR'` → `{ id: 'FR', name: 'France' }`), à enrichir avec une URL canonique, ou à appliquer une logique métier (`Alter::CALL`).
...
Si le modèle cible d'un `Alter::GET` a lui-même un `Alters` qui pointe vers le modèle source, on entre en boucle infinie. En pratique : ne jamais déclarer un `Alter::GET` croisé entre deux modèles. Préférer un modèle "Plain" (`Models::*_PLAIN`) qui n'a pas d'`Alters` du tout, comme cible des lookups — pattern visible dans une application hôte typique (`WAREHOUSES_PLAIN`, `SUBSIDIARIES_SELLERS_PLAIN`).
